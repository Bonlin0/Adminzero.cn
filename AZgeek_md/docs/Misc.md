...
`Shift+Ctrl+p`快捷键调用查询输入栏，输入“cursor”, 可以看到`ctrl+alt+up`与`ctrl+alt+down`可以用于增加多个光标。
...
### 扩散与混淆 (diffussion and confusion)

扩散与混淆是密码学中分组密码的两种基本手段。源自祖师爷 Claude Shannon 在1949年的论文《保密系统的通信理论》。

扩散的目的是将明文、密钥中的每一位的影响散布到尽量多的密文位上。例如DES的加密函数$f$中的选择运算$E$，将32位输入中的每个4每个四位组合为一组，然后将边界的比特用在两个组中重复，实现了明文中比特在密文中的扩散。以及$f$中的$P$置换，将S盒混淆后的8个分组又分散到了不同的分组。

混淆的目的是将明文、密钥与密文的关系复杂化，让密文中的尽可能多的位被密钥中的某一位影响。DES中的S盒提供了混淆的功能。

### 分组密码工作模式

|名称|可否并行|备注|
|-|-|-|
|ECB(Electronic CodeBook) 电码本|可并行| |
|CBC(Cipher-Block Chaining)密文分组链接|不可并行|明文块先与前一个密文块异或|
|CFB(Cipher FeedBack)密文反馈模式|不可并行|加密前一个密文块得到结果作为密钥与明文异或得到密文|
|OFB(Output FeedBack)输出反馈模式|可并行（生成密钥流与异或） |加密前一个组的密钥得到结果作为密钥与明文异或得到密文|
|CTR(CounTeR mode)计数器模式|可并行|将计数通过某种转换得到的结果进行加密，得到结果与明文异或得到密文|
